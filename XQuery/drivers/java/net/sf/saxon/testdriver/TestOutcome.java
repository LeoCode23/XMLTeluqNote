...
public class TestOutcome {
        ...
        try {
            ...
            if (debug && !"all-of".equals(tag) && !"any-of".equals(tag) && !"not".equals(tag)) {
                ...
                if (tag.equals("error")) {
                    ...
                    StringBuilder fsb = new StringBuilder(32);
    ...
    private boolean assertStringValue(XdmNode assertion, SingleResultDoc result, boolean debug) {
        if (isException()) {
            ...
            String resultString;
            String assertionString = assertion.getUnderlyingNode().getStringValue();
            ...
            if (normalizeAtt != null && (normalizeAtt.trim().equals("true") || normalizeAtt.trim().equals("1"))) {
                assertionString = Whitespace.collapseWhitespace(assertionString);
                resultString = Whitespace.collapseWhitespace(resultString);
            }
                ...
                if (debug) {
                    ...
                    for (int i = 0; i < len; i++) {
                        if (resultString.charAt(i) != assertionString.charAt(i)) {
                            driver.println("Actual:'" + StringTool.diagnosticDisplay(resultString) + "'");
                            driver.println("Results differ at index " + i +
                                "(\"" + StringTool.diagnosticDisplay(resultString.substring(i, Math.min(i + 10, len))) + "\") vs (\"" +
                                StringTool.diagnosticDisplay(assertionString.substring(i, Math.min(i + 10, len))) + "\")");
                            break;
                        }
                    }
                }
    ...
    private boolean assertSerialization(XdmNode assertion, SingleResultDoc result, XPathCompiler xpath, boolean debug) throws SaxonApiException {
        if (isException()) {
            ...
            if (normalize) {
                comparand = Whitespace.collapseWhitespace(comparand);
                resultString = Whitespace.collapseWhitespace(resultString);
                    ...
                    for (int i = 0; i < len; i++) {
                        if (resultString.charAt(i) != comparand.charAt(i)) {
                            ...
                            driver.println("Serialized results differ at index " + i +
                                "(\"" + StringTool.diagnosticDisplay(resultString.substring(start, end)) + "\"), expected (\"" +
                                StringTool.diagnosticDisplay(comparand.substring(start, end)) + "\")");
                            break;
                        }
                    }
...
//        }
    ...
    private boolean assertSerializationMatches(XdmNode assertion, SingleResultDoc result, XPathCompiler xpath) {
        if (isException()) {
           ...
           /* String regex = xpath.evaluate(
            ...
            UnicodeString regex = assertion.getUnderlyingNode().getUnicodeStringValue();
            ...
            try {
                ...
                if (re.containsMatch(StringView.of(principalSerializedResult).tidy())) {
                    return true;
                } else {
        ...
        if (isException()) {
           ...
           /* String expectedDoctype = null;
            ...
            try {
                ...
                GroundedValue val0 = SequenceTool.toGroundedValue(iter0);
                SequenceIterator iter1 = expected.axisIterator(Axis.CHILD).next()
                        .getUnderlyingValue().iterateAxis(AxisInfo.CHILD);
                GroundedValue val1 = SequenceTool.toGroundedValue(iter1);
    ...
    private static class RemoveWhitespace implements ItemMappingFunction {
        ...
        public NodeInfo mapItem(Item item) {
            boolean isWhite = ((NodeInfo)item).getNodeKind() == Type.TEXT && Whitespace.isAllWhite(item.getUnicodeStringValue());
            ...
            try {
                ...
                for (XdmItem item : s) {
                    ...
                    AtomicMatchKey comparable = value.isNaN() ?
                            ...
                            value.getXPathMatchKey(collator, context.getImplicitTimezone());
    ...
    private boolean assertDeepEq(XdmNode assertion, SingleResultDoc result, XPathCompiler assertXpc) throws SaxonApiException {
        if (isException()) {
            ...
            s.setVariable(new QName("result"), result.value);
            boolean ok = ((XdmAtomicValue) s.evaluate()).getBooleanValue();
            if (driver.debug && !ok) {
                System.err.println("Actual result: " + result.value);
            }
            return ok;
        }
    }
    ...
    public boolean compareExpectedError(XdmNode assertion) {
        ...
        if (ok) {
            wrongError = null;
        } else if (errorsReported != null && !errorsReported.isEmpty()) {
