...
public class Xslt30TestSuiteDriverHE extends TestDriver {
    ...
    /**
     * Construct a predicate that tests whether a specified attribute of the supplied
     * element has the value "true", "yes", or "1" after whitespace trimming
     * @param attName the name of the attribute
     * @return a function which, when applied to an XDM item, returns true if the item is
     * an element that has an attribute with the given name, and whose whitespace-normalized
     * value is "true", "yes", or "1".
     */
    ...
    public boolean unavailableInXX(String type, String value) {
        ...
        return type.equals("year_component_values") &&
    ...
    protected void runTestCase(XdmNode testCase, XPathCompiler xpath) throws SaxonApiException {
        ...
        // If a static error is expected, suppress JIT compilation so we catch it eagerly
    ...
    private boolean runWithXsltTransformer(XPathCompiler xpath, final TestOutcome outcome, String testName,
        ...
        try {
            ...
            transformer.setResourceResolver(env);
           ...
           /* if (outputUri != null) {
            ...
            transformer.setMessageHandler(message -> {
                outcome.addXslMessage(message.getContent());
        ...
        switch (type) {
            ...
            case "extension-function":
                if (value.equals("Q{http://relaxng.org/ns/structure/1.0}schema-report#1")) {
                    try {
                        ...
                        Object sf = config.getInstance("net.cfoster.saxonjing.SchemaFunction");
                        env.processor.registerExtensionFunction((ExtensionFunctionDefinition) sf);
                        Object sfd = config.getInstance("net.cfoster.saxonjing.SchemaReportFunction");
            ...
            case "year_component_values":
                if ("support year zero".equals(value)) {
                    String oldValue = env.processor.getConfigurationProperty(Feature.XSD_VERSION);
                    env.processor.setConfigurationProperty(Feature.XSD_VERSION, inverse ? "1.0" : "1.1");
                    env.resetActions.add(new Environment.ResetAction() {
                        @Override
                        public void reset(Environment env) {
                            env.processor.setConfigurationProperty(Feature.XSD_VERSION, oldValue);
                        }
                    });
                }
                return !inverse;
    ...
    protected static QName getQNameAttribute(XPathCompiler compiler, XdmNode contextItem, Step<? extends XdmNode> attributePath) {
        ...
        if (att.isPresent()) {
            ...
            if (val.equals("#unnamed")) {
                ...
                try {
                    QNameValue qn = ResolveQName.resolveQName(
                            val, att.get().getUnderlyingNode().getParent());
