...
import static net.sf.saxon.s9api.streams.Steps.atomize;
...
public class QT3TestDriverHE extends TestDriver {
    ...
    public void processSpec(String specStr) {
        ...
        if (reportFileName != null) {
            resultsDoc.setReportFilename(reportFileName);
        }
    }
    ...
    public static boolean isSlow(String test) {
        return false; //test.equals("RangeExpr-409d");
    }
    ...
    public boolean ensureDependencySatisfied(XdmNode dependency, Environment env) {
        ...
        if ("saxon:config".equals(type)) {
            ...
            return true;
        } else if ("saxon:platform".equals(type)) {
            ...
            return ("comment".equals(value) || "text".equals(value)
                            || "processing-instruction".equals(value)
                            || "attribute".equals(value)) == inverse;
        } else if ("revalidation".equals(type)) {
            return !inverse;
            ...
            if ("namespace-axis".equals(value)) {
                ...
                return (edition.equals("PE") || edition.equals("EE")) ^ inverse;
            } else if ("XQUpdate".equals(value)) {
                if (edition.equals("EE") && !inverse) {
                    env.xqueryCompiler.setUpdatingEnabled(true);
                }
                return edition.equals("EE") ^ inverse;
            } else {
    ...
    protected void runTestCase(XdmNode testCase, XPathCompiler xpc) throws SaxonApiException {
        ...
        if (isSlow(testCaseName)) {
            writeTestcaseElement(testCaseName, "tooBig", "test is excessively slow");
            notrun++;
            return;
        }
        ...
        for (XdmNode dependency : dependencies) {
            ...
            if (langVersion.equals("3.0") || langVersion.equals("3.1") || langVersion.equals("4.0")) {
...
//            if (type.equals("feature") && value.equals("XQUpdate")) {
//                updating = true;
//            }
        ...
        if (run && (specOpt == null || specOpt == spec)) {
            XdmValue previousResult = null;
            TestOutcome outcome = new TestOutcome(this);
            for (XdmItem testQuery : testCase.children("test")) {
                String exp = null;
                try {
                    //XdmNode testNode = (XdmNode)testCase.children("test").iterator().next();
                    exp = xpc.evaluate("if (@file) then unparsed-text(resolve-uri(@file, base-uri(.))) else string(.)", testQuery).toString();
                } catch (SaxonApiException err) {
                    println("*** Failed to read query: " + err.getMessage());
                    if (err.getErrorCode().getLocalName().equals("FOUT1190")) {
                        outcome.setException(new QName("", NamespaceConstant.ERR, "XPST0003"));
                    } else {
                ...
                if (outcome.getException() == null) {
                    switch (hostLang) {
                        ...
                        case "XQ": {
                            ...
                            // Is a static base URI defined?
                            if (env.xqueryCompiler != null && env.xqueryCompiler.getBaseURI() == null) {
                                // If not, establish one.
                                // This avoids an NPE in qt3extra test -s:fn-doc -t:fn-doc-101
                                testXqc.setBaseURI(testCase.getBaseURI());
                            }

                            try {
                                // If for some reason there still isn't a base URI, don't NPE here...
                                if (testXqc.getBaseURI() != null) {
                                ...
                                if (previousResult != null) {
                                    if (previousResult instanceof XdmItem) {
                                        selector.setContextItem((XdmItem) previousResult);
                                    } else {
                                        selector.setContextItem(null);
                                    }
                                } else  if (env.contextItem != null) {
                                    selector.setContextItem(env.contextItem);
                                }
                                selector.setURIResolver(new TestURIResolver(env));
                                if (env.unparsedTextResolver != null) {
                                ...
                                if (isTrue("update").test(testQuery) && q.isUpdateQuery()) {
                                    selector.run();
                                    XdmNode resultDoc = null;
                                    int numberOfUpdatedDocs = 0;
                                    Iterator<XdmNode> iter = selector.getUpdatedDocuments();
                                    while (iter.hasNext()) {
                                        numberOfUpdatedDocs++;
                                        resultDoc = iter.next();
                                    }
                                    if (resultDoc == null) {
                                        // Some test queries don't actually update anything, in which case
                                        // getUpdatedDocuments() returns nothing. The tests are written on
                                        // the assumption that we process the "main input" of the previous
                                        // step, which is either the context item or the value of the
                                        // variable $input-context. Very pragmatic code here...
                                        if (previousResult == null) {
                                            resultDoc = (XdmNode)env.params.get(new QName("input-context"));
                                        } else {
                                            resultDoc = (XdmNode) previousResult;
                                        }
                                        if (resultDoc == null) {
                                            outcome.setPrincipalResult(XdmEmptySequence.getInstance());
                                        } else {
                                            outcome.setPrincipalResult(resultDoc);
                                        }
                                    }
                                    if (numberOfUpdatedDocs == 1) {
                                        previousResult = resultDoc;
                                        outcome.setPrincipalResult(resultDoc);
                                    }
                                } else {
                                    if (needSerializedResult) {
            ...
            if (!outcome.isException() && previousResult != null) {
                outcome.setPrincipalResult(previousResult);
            }
    ...
    private static boolean isApplicableToSpecVersion(String value, Spec spec) {
        ...
        if (!value.contains(spec.shortSpecName)) {
            ...
            applicable = true;
                    ...
                    || spec.specAndVersion.equals("XQ40")) &&
                (value.contains("XQ10+") || value.contains("XQ30+") || value.contains("XQ31+"))) {
            applicable = true;
        } else if ((spec.specAndVersion.equals("XP30") || spec.specAndVersion.equals("XP31") || spec.specAndVersion.equals("XP40")) &&
                (value.contains("XP20+") || value.contains("XP30+") || value.contains("XP31+"))) {
            applicable = true;
        }
        return applicable;
    }
            ...
            if (locations.length == 0) {
                try {
                    ...
                    for (int i = 0; i < files.size(); i++) {
                        ...
                        ss[i] = new StreamSource(new FileInputStream(file), uri.toString());
                    }
                    return ss;
